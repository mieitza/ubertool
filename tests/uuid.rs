...
fn uuid_new_default_is_v4() {
    let out = Command::cargo_bin("ubertool").unwrap()
        .args(["uuid", "new"]).assert().success().get_output().stdout.clone();
...
fn uuid_new_v7_emits_time_ordered_uuid() {
    let out = Command::cargo_bin("ubertool").unwrap()
        .args(["uuid", "new", "--version", "7"]).assert().success().get_output().stdout.clone();
    let s = String::from_utf8(out).unwrap();
    let s = s.trim();
    let parts: Vec<&str> = s.split('-').collect();
...
fn uuid_new_json_mode() {
    let out = Command::cargo_bin("ubertool").unwrap()
        .args(["--json", "uuid", "new"]).assert().success().get_output().stdout.clone();
...
fn uuid_new_invalid_version_is_usage_error() {
    Command::cargo_bin("ubertool").unwrap()
        .args(["uuid", "new", "--version", "99"]).assert().failure().code(2);
}
