...
fn chmod_calc_755() {
    Command::cargo_bin("ubertool").unwrap()
        .args(["chmod", "calc", "rwxr-xr-x"]).assert().success()
        .stdout("755\n");
}
...
fn chmod_calc_644() {
    Command::cargo_bin("ubertool").unwrap()
        .args(["chmod", "calc", "rw-r--r--"]).assert().success()
        .stdout("644\n");
}
...
fn chmod_calc_777() {
    Command::cargo_bin("ubertool").unwrap()
        .args(["chmod", "calc", "rwxrwxrwx"]).assert().success()
        .stdout("777\n");
}
...
fn chmod_parse_755() {
    let out = Command::cargo_bin("ubertool").unwrap()
        .args(["--json", "chmod", "parse", "755"]).assert().success().get_output().stdout.clone();
